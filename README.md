## 👋 About Me

- Primary language: C#  
- Working on too many projects — always learning  
- Developer of **Color-Chan**  

## 🔭 Current Projects

- **Color-Chan / [Color-Chan.Discord](https://github.com/Color-Chan/Color-Chan.Discord)** — A Discord bot written in C# with an custom open source Discord Library.
- **[Serilog.Sinks.Mongodb.TimeSeries](https://github.com/BrammyS/Serilog.Sinks.Mongodb.TimeSeries)** — A time-series sink implementation for MongoDB logging  
- **[necesse-docker-server](https://github.com/BrammyS/necesse-docker-server)** — Docker image for self-hosting a Necesse game server  
- **[trx-tools](https://github.com/BrammyS/trx-tools)** — Tools for reporting / merging TRX (test result) files  
- **[Discord-Bot-Template](https://github.com/BrammyS/Discord-Bot-Template)** — Boilerplate for Discord bots using slash commands and buttons  
- **[GalleryPixels](https://github.com/BrammyS/GalleryPixels)** — A self-hosted catalog for photography & videography
- **[B# Language](https://github.com/BrammyS/B-Sharp)** - B#, a custom language using Jasmin bytecode
- **[Docker Terraria Server](https://github.com/BrammyS/Terraria)** - Docker images for a vanilla Terraria server - graceful shutdown and multi-arch 
